...
public sealed class ValidationSubsystem : CliSubsystem
{
    ...
    private void ValidateValue(CliValueSymbol symbol, ValidationContext validationContext)
    {
        var valueConditions = symbol.GetValueConditions();
        if (valueConditions is null)
        {
            return; // nothing to do
        }

        var value = validationContext.PipelineResult.GetValue(symbol);
        var valueResult = validationContext.ParseResult?.GetValueResult(symbol);
        foreach (var condition in valueConditions)
        {
            ValidateValueCondition(value, valueResult, condition, validationContext);
        }
    }
    ...
    private IEnumerable<CliValueSymbol> GetValueSymbols(IEnumerable<CliCommandResult> commandResults)
    {
        throw new NotImplementedException();
    }

    // Consider moving to CliCommandResult
    private IEnumerable<CliCommandResult> CommandAndAncestors(CliCommandResult commandResult)
    {
        throw new NotImplementedException();
    }

    private IEnumerable<ParseError>? ValidateValueCondition(object? value, CliValueResult valueResult, ValueCondition condition, ValidationContext validationContext)
    {
        Validator? validator = GetValidator(condition);
        return validator switch
        {
            null => null, // silence NRT
            ValueValidator valueValidator => valueValidator.Validate(value, valueResult, condition, validationContext),
            _ => throw new InvalidOperationException("Validator must be derive from CommandValidator")
        };
    }
    ...
    private IEnumerable<ParseError>? ValidateCommandCondition(CliCommandResult commandResult, ValueCondition condition, ValidationContext validationContext)
    {
        Validator? validator = GetValidator(condition);
        return validator switch
        {
            null => null, // silence NRT
            CommandValidator commandValidator => commandValidator.Validate(commandResult, condition, validationContext),
            _ => throw new InvalidOperationException("Validator must be derive from CommandValidator")
        };
    }
